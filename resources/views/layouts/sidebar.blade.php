...
<aside id="main-sidebar" x-data="{ showLogoutModal: false }" style="font-family: 'Inter', sans-serif;" class="sidebar-transition w-[280px] bg-gradient-to-b from-[#003d7a] to-pelindo-navy flex flex-col fixed lg:relative inset-y-0 left-0 z-[90] shadow-[10px_0_40px_rgba(0,45,93,0.1)] shrink-0">
    ...
    <div class="h-[88px] flex items-center px-8 shrink-0 relative overflow-hidden">
        ...
        <img src="{{ asset('pelindo.png') }}" alt="Pelindo Logo" class="h-8 md:h-9 object-contain grayscale brightness-0 invert relative z-10">
    </div>
    ...
    <div class="mt-auto shrink-0 pb-2">
        @auth
            <div class="p-4 flex items-center justify-between gap-3">
                ...
                <button type="button" @click="showLogoutModal = true" title="Akhiri Sesi" class="w-9 h-9 flex items-center justify-center rounded-xl bg-white/5 text-slate-400 hover:bg-red-500 hover:text-white hover:shadow-lg hover:shadow-red-500/20 transition-all duration-300 group">
                    ...
                    <div x-show="showLogoutModal" x-cloak
                        ...
                        <div @click.away="showLogoutModal = false"
                            ...
                            <div class="p-10 text-center">
                                <div class="w-20 h-20 bg-amber-50 dark:bg-amber-900/30 text-amber-600 rounded-full flex items-center justify-center text-4xl mx-auto mb-6 border border-amber-100 dark:border-amber-800">
                                    <i class="fas fa-sign-out-alt"></i>
                                </div>
                                <h3 class="text-2xl font-black text-[#003366] dark:text-white uppercase tracking-tight mb-2">Akhiri Sesi?</h3>
                                ...
                                <div class="flex gap-4">
                                    <button @click="showLogoutModal = false" class="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">Batal</button>
                                    <form method="POST" action="{{ route('logout') }}" class="flex-1">
                                        @csrf
                                        <button type="submit" class="w-full py-4 bg-[#003366] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-900/20 hover:bg-[#002244] transition-all">Keluar Sesi</button>
                                    </form>
                                </div>
                            </div>
                        </div>
