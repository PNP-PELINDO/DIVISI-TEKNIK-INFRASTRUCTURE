    ...
    <div class="max-w-4xl mx-auto w-full space-y-6 animate-fade-up">
        ...
        <div class="bg-white dark:bg-slate-900 p-8 md:p-10 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
            ...
            <div class="mb-10 flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-black text-[#003366] dark:text-blue-400 uppercase tracking-tight">Registrasi Unit Aset</h2>
                ...
                <div class="w-16 h-16 bg-blue-50 dark:bg-blue-900/30 text-[#0055a4] dark:text-blue-400 rounded-2xl flex items-center justify-center text-3xl border border-blue-100 dark:border-blue-800 shadow-inner">
            ...
            <form action="{{ route('admin.infrastructures.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8" 
                ...
                <!-- UPLOAD FOTO -->
                <div onclick="document.getElementById('image-input').click()" class="bg-slate-50 dark:bg-slate-800/50 p-8 rounded-3xl border-2 border-dashed border-slate-300 dark:border-slate-700 group hover:border-[#0055a4] dark:hover:border-blue-400 hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-all cursor-pointer">
                    ...
                    <div class="flex flex-col items-center">
                        <div id="image-preview-container" class="hidden mb-4">
                            ...
                            <p class="text-[9px] text-blue-600 dark:text-blue-400 font-black uppercase text-center mt-4 tracking-widest">Pratinjau Foto Terpilih</p>
                        </div>
                        
                        <div id="upload-placeholder" class="text-center py-4">
                            <div class="w-20 h-20 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-slate-700 shadow-sm group-hover:scale-110 group-hover:text-blue-600 transition-all">
                                <i class="fas fa-cloud-upload-alt text-3xl text-slate-300 dark:text-slate-600 group-hover:text-blue-500 transition-colors"></i>
                            </div>
                ...
                <div>
                    ...
                    @else
                        ...
                        <p class="text-[9px] font-bold text-emerald-500 dark:text-emerald-400 mt-2 ml-1 uppercase tracking-widest"><i class="fas fa-shield-alt mr-1"></i> Data akan otomatis terhubung ke area {{ auth()->user()->entity->name ?? 'Anda' }}</p>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <div class="relative">
                            <select name="category" x-model="selectedCategory" class="w-full border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 rounded-2xl text-sm font-black p-5 focus:ring-4 focus:ring-blue-50 dark:focus:ring-blue-900/20 focus:border-[#003366] dark:focus:border-blue-400 transition-all uppercase appearance-none cursor-pointer text-slate-700 dark:text-slate-200" required>
                    ...
                    <div>
                        <label class="block text-[11px] font-black {{ $errors->has('code_name') ? 'text-red-600 dark:text-red-400' : 'text-[#0055a4] dark:text-blue-400' }} uppercase tracking-[0.2em] mb-3 ml-1">
                    ...
                    <div x-show="inputMode === 'select'" class="space-y-4">
                        <div class="relative">
                            <select name="type_select" x-model="selectedType" 
                                    ...
                                    " 
                                    class="w-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-2xl text-sm font-black p-5 focus:ring-4 focus:ring-blue-50 dark:focus:ring-blue-900/20 focus:border-[#003366] dark:focus:border-blue-400 transition-all uppercase appearance-none cursor-pointer text-slate-700 dark:text-slate-200">
                                <option value="">-- Pilih dari Basis Data --</option>
                                @foreach($typeCategoryMap ?? [] as $type => $cat)
                                    <option value="{{ $type }}">{{ $type }}</option>
                                @endforeach
                                <option value="new" class="text-blue-600 dark:text-blue-400">➕ INPUT JENIS BARU (CUSTOM)</option>
                            </select>
                            <i class="fas fa-chevron-down absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-xs"></i>
                        </div>
                    ...
                    <div x-show="inputMode === 'text'" x-cloak class="space-y-4">
                        <div class="flex flex-col sm:flex-row gap-4">
                            <input type="text" name="type_new" value="{{ old('type_new') }}" placeholder="Ketik jenis alat baru (Contoh: Rubber Tyred Gantry)..." 
                                   class="flex-1 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-2xl text-sm font-black p-5 focus:ring-4 focus:ring-blue-50 dark:focus:ring-blue-900/20 focus:border-[#003366] dark:focus:border-blue-400 transition-all uppercase text-slate-900 dark:text-slate-100">
                        ...
                        <p class="text-[9px] font-bold text-blue-600 dark:text-blue-400 ml-1 uppercase tracking-widest"><i class="fas fa-plus-circle mr-1"></i> Anda sedang menambahkan jenis alat baru ke sistem</p>
                    </div>
                </div>

                <!-- SUBMIT BUTTONS -->
                <div class="pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row gap-4">
                    <button type="submit" class="flex-1 bg-[#003366] dark:bg-blue-600 hover:bg-[#001e3c] dark:hover:bg-blue-700 text-white py-5 rounded-2xl text-xs font-black uppercase tracking-[0.2em] shadow-2xl shadow-blue-900/20 transition-all flex items-center justify-center gap-3 active:scale-[0.98]">
                        <i class="fas fa-save text-blue-400"></i> Simpan Inventaris Unit
                    </button>
