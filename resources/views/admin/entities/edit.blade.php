<x-app-layout>
    <div class="max-w-3xl mx-auto w-full space-y-8 pb-16 animate-fade-up">
        ...
        <div class="bg-white p-8 md:p-10 rounded-[2rem] border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <div class="mb-8">
                <h2 class="text-2xl font-black text-slate-800 uppercase tracking-tight">Edit Data Entitas</h2>
            ...
            <form action="{{ route('admin.entities.update', $entity->id) }}" method="POST" class="space-y-6">
                ...
                @method('PUT') <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $entity->name) }}" class="w-full border-slate-200 bg-slate-50 rounded-xl text-sm font-bold p-4 focus:ring-4 focus:ring-amber-50 focus:border-amber-500 transition-all uppercase" required>
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $entity->code) }}" class="w-full border-slate-200 bg-slate-50 rounded-xl text-sm font-bold p-4 focus:ring-4 focus:ring-amber-50 focus:border-amber-500 transition-all uppercase" required>
